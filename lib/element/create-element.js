...
const VALID_CHILDREN = ['string', 'element', 'array']
...
const separateArgs = args => {
  ...
  if (args.length === 2) {
    if (VALID_CHILDREN.includes(typeOf(args[1]))) {
...
export default (...args) => {
  const [selector, props, children] = separateArgs(args)
  ...
  validateArguments({
    ...
    expectedTypes: [
      'string',
      'object',
      children === false ? 'optional' : VALID_CHILDREN
    ]
  })
  ...
  Object.keys(props).forEach(prop => {
    ...
    if (prop.substring(0, 2) === 'on') {
      const eventName = prop.substring(2, prop.length)
